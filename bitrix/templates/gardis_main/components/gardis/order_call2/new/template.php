...
<div class="form_grid form_callback" id="phone_take" 11>
    ...
    <? if (strlen($arResult["OK_MESSAGE"]) > 0): ?>
        <div class="errors">
            ...
            <script>var _gaq = _gaq || []; _gaq.push(['_trackEvent', 'форма', 'отправлена', 'заказать звонок']);yaCounter24428276.reachGoal('ORDER_CALL_SUBMITED');</script>
            ...
            <a href="javascript:;" onclick="window.location.href='<?= $APPLICATION->GetCurPageParam('order_call=y', array('success', 'AJAX_CALL', 'bxajaxid', 'order_call',
                'ask_price', 'ask_question'))?>'">отправить ещё сообщение</a>
        </div>
    <?else:?>
        <div class="clear"></div>
        <script>
        ...
        <form id="order_call_form_id" action="<?=$APPLICATION->GetCurPage()?>" method="POST">
            <fieldset>
                <?= bitrix_sessid_post() ?>                <?php /*	<label><?=GetMessage("MFT_NAME")?>:<?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("NAME", $arParams["REQUIRED_FIELDS"])):?><span class="mf-req">*</span><?endif?></label>
