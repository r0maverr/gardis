...
<div class="order-call fancy-order-call make-pad">
<?if(!empty($arResult["FORM_NOTE"])){?>
	<script>
	$(document).ready(function(){
		$('.form2__html').css({display:'none'});
		$('.order-call').removeClass('make-pad');
...
$(document).ready(function(){
	...
	$('.onClickOrderCall').on('click', function(){
		$('.form2__html').css({display:'block'});
		$('.order-call').addClass('make-pad');
		$('.form_success2').css({display:'none'});
        $('.form_success1').css({display:'none'});
		formbox = $(".fancy-order-call");
