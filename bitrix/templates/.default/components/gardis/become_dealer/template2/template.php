...
<div class="form_grid">
	...
	<?if (strlen($arResult["OK_MESSAGE"]) > 0):?>
		<div class="errors">
<?$APPLICATION->AddHeadString("<script>window.onload = function(){yaCounter24428276.reachGoal('CONTACT_DIRECTOR_SUBMITED');}; var _gaq = _gaq || [];_gaq.push(['_trackEvent', 'форма', 'отправлена', 'вопрос директору']);</script>")?>
			<div class="mf-ok-text"><?=$arResult["OK_MESSAGE"]?></div>
	...
	<?else:?>
		...
		<form action="<?=$APPLICATION->GetCurPage()?>" method="POST">
			<fieldset>
				...
				<input type="submit" class="grey_nice_button on_right" name="submit" value="<?=$arParams["BUTTON_TITLE"];?>" onclick="_gaq.push(['_trackEvent', 'форма', 'отправка', 'вопрос директору']); yaCounter24428276.reachGoal('CONTACT_DIRECTOR_SUBMIT'); return true;">
			</fieldset>
		</form>
	<?endif;?>
</div>