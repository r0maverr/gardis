...
foreach($arResult['ITEMS'] as $keyItem => $arItem){
    foreach($arItem['PROPERTIES']['GOODS']['VALUE'] as $keyVal => $GoodId)
    {
        $res = CIBlockSection::GetByID($GoodId);
        $ar_res = $res->GetNextElement();
        $ar_fields = $ar_res->GetFields();
        $arResult['ITEMS'][$keyItem]['PROPERTIES']['GOODS']['GOOD_NAME'][$keyVal]=$ar_fields['NAME'];
        $arResult['ITEMS'][$keyItem]['PROPERTIES']['GOODS']['URL'][$keyVal]=$ar_fields['SECTION_PAGE_URL'];
        $img = CFile::GetByID($ar_fields['PICTURE'])->Fetch();
        $arResult['ITEMS'][$keyItem]['PROPERTIES']['GOODS']['IMG'][$keyVal]=$ar_fields['SECTION_PAGE_URL'] = empty($img) ? '' : '/upload/'.$img["SUBDIR"].'/'.$img["FILE_NAME"];
        //echo $GoodId.'__'.$arResult['ITEMS'][$keyItem]['PROPERTIES']['GOODS']['IMG'][$keyVal].'__'.$ar_fields['NAME'].'<br>';
    }
    //echo print_r($arItem['PROPERTIES']['GOODS']);
}
