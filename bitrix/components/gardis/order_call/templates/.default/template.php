<? if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die(); ?>
	<table style="margin-top: 2px; float: right; height: 25px; vertical-align: middle;">
		<tr> <?php /* <td ><a id="bxid_745511" class="fancybox" style="border-bottom: 1px dotted white" href="#phone_take" ><img id="bxid_849199" src="/upload/phone.png" alt="Телефон" width="20" height="20"  /></a></td> */ ?>
			<td style="vertical-align: top;">&nbsp; <a href="#phone_take" class="fancybox order_call_button"
...
<? endif; ?>        <? if (strlen($arResult["OK_MESSAGE"]) > 0): ?>
	<div class="errors">
		<?$APPLICATION->AddHeadString("<script>var _gaq = _gaq || []; _gaq.push(['_trackEvent', 'all_pages', 'request_call_submit', , , true]);</script>")?>
		<div class="mf-ok-text"><?= $arResult["OK_MESSAGE"] ?></div>
...
<? else: ?>
	...
	<script>
		$(document).ready(function () {
            ...
            $('#order_call_form_id input[name="user_phone"]').mask('+7 (999) 999-99-99', {placeholder: '+7 (___) ___-__-__'});
		});
	</script>
	<form id="order_call_form_id" action="<?=$APPLICATION->GetCurPage()?>" method="POST">
		<fieldset>                    <?= bitrix_sessid_post() ?>                <?php /*	<label><?=GetMessage("MFT_NAME")?>:<?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("NAME", $arParams["REQUIRED_FIELDS"])):?><span class="mf-req">*</span><?endif?></label>
			...
			?>                    <input type="hidden" name="PARAMS_HASH" value="<?=$arResult["PARAMS_HASH"]?>"> <input type="submit"
		                                                                                   ...
		                                                                                   onclick="yaCounter24428276.reachGoal('ORDER_CALL_SUBMIT<?= (isset($_GET['yclid']) || isset($_COOKIE['gardies-yclid-phone'])) ? '_DIRECT' : '' ?>'); return true;">
		</fieldset>
	</form>    
</div>
<?endif; ?>