...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] <> '' && (!isset($_POST["PARAMS_HASH"]) || $arResult["PARAMS_HASH"] === $_POST["PARAMS_HASH"]))
{
	...
	if(check_bitrix_sessid())
	{
		if(empty($arParams["REQUIRED_FIELDS"]) || !in_array("NONE", $arParams["REQUIRED_FIELDS"]))
		{
			...
			if((empty($arParams["REQUIRED_FIELDS"]) || in_array("TOWN", $arParams["REQUIRED_FIELDS"])) && strlen($_POST["user_town"]) <= 1)
				$arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_TOWN");
...
//				"SUBJECT_PREFIX" => ((isset($_GET['yclid']) || isset($_COOKIE['gardies-yclid-phone'])) ? 'Директ_' : 'Сео_')
			...
			$arProp["ADRESS"] = $_POST["user_town"];
			$arProp["TYPE"] = 4; //запрос цены
			$arLoadProductArray = Array(  
				"IBLOCK_ID"      => 26,
				"NAME"           => $_POST["user_name"]." - ".$_POST["user_email"],
				"ACTIVE"         => "Y",
				"DETAIL_TEXT"    => "Интересует: " . $_POST["PRODUCT_NAME"] . "\n\r" . $_POST["MESSAGE"],
		...
		$arResult["AUTHOR_TOWN"] = htmlspecialcharsEx($_POST["user_town"]);
