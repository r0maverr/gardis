...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		if (isset($arParams['PARENT_SECTION_CODE']) && strlen($arParams['PARENT_SECTION_CODE']) > 0)
		{
      $arSelectSection = array();

      if(preg_match("/^UF_/", $arParams["META_KEYWORDS"])) $arSelectSection[] = $arParams["META_KEYWORDS"];
      if(preg_match("/^UF_/", $arParams["META_DESCRIPTION"])) $arSelectSection[] = $arParams["META_DESCRIPTION"];
      if(preg_match("/^UF_/", $arParams["BROWSER_TITLE"])) $arSelectSection[] = $arParams["BROWSER_TITLE"];
      
      $arSelectSection[] = GARDIES_CATALOG_SECTION_GALLERY_ID_PROP;
      $arSelectSection[] = GARDIES_CATALOG_SECTION_GALLERY_TEXT_PROP;
        ...
        $arResult['SECTION_FIND'] = $arSection;
        
        // если есть привязка к галерее, то получаем ссылку на неё и первое изображение
        $arResult['SECTION_FIND']['GALLERY'] = false;
        if (isset($arResult['SECTION_FIND'][GARDIES_CATALOG_SECTION_GALLERY_ID_PROP])
        && $arResult['SECTION_FIND'][GARDIES_CATALOG_SECTION_GALLERY_ID_PROP] > 0) {
          $db = CIBlockElement::GetList(
          array('SORT' => 'ASC'), 
          array(
          'ACTIVE' => 'Y', 
          'SECTION_ID' => $arResult['SECTION_FIND'][GARDIES_CATALOG_SECTION_GALLERY_ID_PROP],
          'IBLOCK_ID' => GARDIES_GALLERY_IBLOCK_ID,
          ...
          if ($res = $db->GetNext()) {
            $arResult['SECTION_FIND']['GALLERY']['TEXT'] = $arResult['SECTION_FIND'][GARDIES_CATALOG_SECTION_GALLERY_TEXT_PROP];
            $arResult['SECTION_FIND']['GALLERY']['URL'] = $res['DETAIL_PAGE_URL'];
            $arResult['SECTION_FIND']['GALLERY']['PICTURE'] = CFile::ResizeImageGet(
            CFile::GetFileArray($res['DETAIL_PICTURE']), 
...
if(isset($arResult["ID"]))
{
  ...
  if (isset($arResult['SECTION_FIND'][$arParams["META_KEYWORDS"]]))
  {
    $val = $arResult['SECTION_FIND'][$arParams["META_KEYWORDS"]];
    if (is_array($val))
      $val = implode(" ", $val);
    $APPLICATION->SetPageProperty("keywords", $val);
  }
  
  if (isset($arResult['SECTION_FIND'][$arParams["META_DESCRIPTION"]]))
  {
    $val = $arResult['SECTION_FIND'][$arParams["META_DESCRIPTION"]];
    if (is_array($val))
      $val = implode(" ", $val);
    $APPLICATION->SetPageProperty("description", $val);
  }

  if(isset($arResult['SECTION_FIND'][$arParams["BROWSER_TITLE"]]))
  {
    $val = $arResult['SECTION_FIND'][$arParams["BROWSER_TITLE"]];
    if (is_array($val))
      $val = implode(" ", $val);
