...
$kolKrepegForHeightOgr = array( // К – соответствие кол-ва крепежей высоте ограждения, см. страницу. 24-26
    '430' => 2,
    '630' => 2,
    '640' => 2,
    '830' => 2,
...
if($arPost['color'] == 'цинк'){
    ...
    if($arPost['color'] == 'other_color'){
        $colorValueComment = '#Нестандартный цвет# '.$arPost['color_your'];
        $colorValue = 'RAL-6005';
    }
    else{
        $colorValue = $arPost['color'];
...
// p($widthVorot,'widthVorot');
    ...
    elseif($arPost['kreplenie'] == 'planka_prigim' || $arPost['kreplenie'] == 'planka_samores') // планка
    {
        for($i=1; $i<=$arPost['cntStoron']; $i++)
        {
            ...
            $countStolbPaneli = $countPanels - $arPost['kol_kalitka'] - $arPost['kol_vorot'] + 1; // Количество столбов=количество панелей+1=
            $countKrepegPlanka = $countStolbPaneli * $kolKrepegForHeightOgr[$height_paneli]; // Количество крепежей=количество столбов х К= 57х3=171
...
//p($countKrepegKKPNabor, 'количество крепежей прямых в наборе');
            ...
            if($sechenieStolbaPodVorota == '100*100')
                $countKrepegKKK = $kolKrepegNaKalitku[$arPost['height_kalitka']*10] * (int)$arPost['kol_kalitka']; // Количество крепежей концевых (ККК)=Кв х количество ворот+Кв х количество калиток =8х1+8х1=16
            else
...
/*$arParams['PANELI'][] = array( // массив панелей
    ...
    foreach($arParams['PANELI'] as $arPanel)
    {
        ...
        if($arPost['kreplenie'] == 'homut')
        {
            ...
            {
        //p($arEl);
                if( strpos($arEl['NAME'], 'без логотипа') !== false) // пропускаем
                    continue;
        ...
        elseif($arPost['kreplenie'] == 'planka_samores'){// получим Комплект крепления КК.DKC
                ...
                $arFilter = array(
                    ...
                    'SECTION_CODE' => 'komplekty_krepezha', 
                    'INCLUDE_SUBSECTIONS' => 'Y',  
                    'NAME' => 'Комплект крепления КК.DKC',
                );
                ...
                if(count($returnRes['KKDKC']['ELEM']) == 0){
                    $returnRes['KKDKC']['ERROR'][] = 'Комплект крепления КК.DKC не найден';
                } 
            }
        else{
           
            // получим планку прижимную
            $arFilter = array(
                'IBLOCK_ID' => $idIBlock,  
                'ACTIVE' => 'Y',
                'SECTION_CODE' => 'komplekty_krepezha', 
            ...
            // добавим метизы
            // Если мы выбираем крепление планка прижимная, то к ней автоматически должны считаться метизы для столбов 60х60 или 80х80. Если выбран столб 60х40, то все равно считаем комплект метизов для столбов 60х60
            $arFilter = array(
                'IBLOCK_ID' => $idIBlock, 
                'ACTIVE' => 'Y',
                'INCLUDE_SUBSECTIONS' => 'Y',  
            ...
            if($sechenieStolbaPaneli == '60*60' || $sechenieStolbaPaneli == '60*40'){
                $arFilter['NAME'] = 'Комплект метизов к планке прижимной ПП.40.30 на столбы сечением 60х60';
            }
            elseif($sechenieStolbaPaneli == '80*80'){
                $arFilter['NAME'] = 'Комплект метизов к планке прижимной ПП.40.30 на столбы сечением 80х80';
            }
    //p($arFilter, 'arFilter');
            if($arFilter['NAME'])
            {
                ...
                if(count($returnRes['komplekt_metizov']['ELEM']) == 0){
                    $returnRes['komplekt_metizov']['ERROR'][] = $arFilter['NAME'].' не найден';
                }  
            } 
        }
    ...
    if($arPost['egoza_sbb'] == 'on'){// периметр
        ...
        for($i=0; $i < $cntSbb; $i++)
        {
            ...
            while ($arEl = $res->GetNext())
            {
                ...
                // добавим Проволока оцинкованная 2.5мм.1.26
                $arFilterProvoloka = array(
                    'IBLOCK_ID' => $idIBlock,  
                    'ACTIVE' => 'Y',
                    //'SECTION_CODE' => 'komplekty_krepezha', 
                    'INCLUDE_SUBSECTIONS' => 'Y',  
                    'NAME' => 'Проволока оцинкованная 2.5мм.1.26',
                );
                ...
                if(count($returnRes['PROVOLOKA']['ELEM']) == 0){
                    $returnRes['PROVOLOKA']['ERROR'][] = 'Проволока оцинкованная 2.5мм.1.26 не найдена';
                } 
            }
...
p($coefficientForKozirek, 'coefficientForKozirek');
    ...
    if($arPost['kozirek'] == 'on'){
        ...
        // добавляем  Если мы заказываем козырьки (козырьковую панель), на нее обязательно должны считаться крепления.  
        $arFilter = array(
            'IBLOCK_ID' => $idIBlock, 
            'ACTIVE' => 'Y',
            'INCLUDE_SUBSECTIONS' => 'Y',  
        ...
        if($sechenieStolbaPaneli == '60*60'){
            $arFilter['NAME'] = 'Комплект крепления козырьковой панели КККП.60.60.20.ОЦ полимер';
        }
        elseif($sechenieStolbaPaneli == '80*80'){
            $arFilter['NAME'] = 'Комплект крепления козырьковой панели КККП.80.80.20.ОЦ полимер';
        }
p($arFilter, 'arFilter');
        if($arFilter['NAME'])
        {
            ...
            if(count($returnRes['komplekt_krepl_dly_kozirka']['ELEM']) == 0){
                $returnRes['komplekt_krepl_dly_kozirka']['ERROR'][] = $arFilter['NAME'].' не найден';
            }  
        }
    }
    ...
    if($arPost['antivand_krepeg'] == 'on' && $arPost['kreplenie'] != 'planka_samores'){
        ...
        if($arPost['kreplenie'] == 'planka_prigim')
            $arFilter['NAME'] = 'гайка М8 антиванд.';
        $arSelect = array('ID', 'IBLOCK_ID', 'NAME');   
        $res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
